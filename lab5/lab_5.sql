...
SELECT r.number, c.name, rib.checkout_date
FROM client c
         ...
         INNER JOIN (SELECT r.id_room, MAX(checkout_date) AS last_checkout_date
                     FROM room_in_booking rib
                              INNER JOIN room r ON rib.id_room = r.id_room
                              INNER JOIN hotel h ON r.id_hotel = h.id_hotel
                     WHERE h.name = 'Космос' &&
                           MONTH(rib.checkout_date) = 4
                     GROUP BY r.id_room) AS sr ON r.id_room = sr.id_room && rib.checkout_date = sr.last_checkout_date;
...
UPDATE room_in_booking rib
    ...
    INNER JOIN room_category rc ON r.id_room_category = rc.id_room_category
    INNER JOIN hotel h ON r.id_hotel = h.id_hotel
SET rib.checkout_date = rib.checkout_date + INTERVAL 2 DAY
WHERE h.name = 'Космос' &&
      rc.name = 'Бизнес' &&
